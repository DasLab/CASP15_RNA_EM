...
def score_all(pdbs,out_file_prefix,emmap,native,resolution,threshold,usalign_exec,chimerax_exec,EM=True,phenix_location=""):
    ...
    for pdb in pdbs:
        ...
        if EM:
            ...
            # TODO add all scores, maybe option of score to not do?
    ...
    df.index.name = 'model'
    df.to_csv(f"{out_file_prefix}_scores.csv")
...
# parse_avg_Q.py trimmedQ.pdb
# outputs average Q score 
# saves a csv for per res Q scores trimmedQ.pdb_PER_RES_SUMMARY.out
...
df = ppdb.df['ATOM']
df_heavy = df[df.element_symbol!='H']
...
print('Average Q:',df_heavy.b_factor.mean())
print('Average Q backbone:',df_heavy_backbone.b_factor.mean())
print('Average Q base:',df_heavy_base.b_factor.mean())
...
print('Average Q backbone pos:',df_heavy_backbone.b_factor.mean())
print('Average Q base pos:',df_heavy_base.b_factor.mean())

per_nuc_Q = df_heavy.rename(columns={'b_factor':'Q'}).groupby('residue_number').mean().Q
per_nuc_Q_backbone = df_heavy_backbone.rename(columns={'b_factor':'Q_backbone'}).groupby('residue_number').mean().Q_backbone
per_nuc_Q_base = df_heavy_base.rename(columns={'b_factor':'Q_base'}).groupby('residue_number').mean().Q_base
per_nuc_df = pd.concat([per_nuc_Q,per_nuc_Q_backbone,per_nuc_Q_base],names=['1','2','3'], axis=1)
per_nuc_df.to_csv(f'{sys.argv[1].split(\"_phenixDOCKED\")[0]}_Q_PER_RES_SUMMARY.out',sep=' ')
...
def run_tempy(pdb,emmap,resolution):
    ...
    for chain in chains:
        for res,score in SMOC[0][chain].items():
            score['tempy_smoc_per_residue'].append(score)
            score['tempy_smoc_1_per_residue'].append(score)
    score_dict['tempy_smoc'] = sum(score['tempy_smoc_per_residue']) / len(score['tempy_smoc_per_residue'])
    score_dict['tempy_smoc1'] = sum(score['tempy_smoc_1_per_residue']) / len(score['tempy_smoc_1_per_residue'])
    return score_dict

def run_lga(pdb,native,lga_location='',atom='C4,',result_file=None):
    if result_file is None:
        result_file = f'{pdb.rsplit(".",1)[0]}_{native.rsplit(".",1)[0].rsplit("/",1)[-1]}_LGA.out'
    system(f'ulimit -s unlimited; {lga_location}lga')
    command = [f'{lga_location}runlga.mol_mol.pl', pdb, native, '-3', f'-atom:{atom}', '-gdc', '-stral', '-o2']
    out_file = open(result_file, 'w')
    p = sp.Popen(command, stdout=out_file, stderr=sp.PIPE)
    out, err = p.communicate()
    if p.returncode:
...
def dock_pdb_phenix_iterative(pdb,emmap,resolution,phenix_location='',output_pdb=None):
    ...
    if path.isfile(output_pdb): remove(output_pdb)
    ...
    if path.isfile(output_pdb): return {'docked':'phenix'}
    ...
    if path.isfile(output_pdb): return {'docked':'phenix'}
    dock_pdb_phenix(pdb,emmap,resolution,phenix_location=phenix_location,output_pdb=output_pdb,skip_if_low_cc=False,min_cc=0.01)
    if path.isfile(output_pdb): return {'docked':'failed-phenix'}
