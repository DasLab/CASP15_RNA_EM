# ChimeraX --nogui --script 'atom_inclusion_chimerax.py MRC DOCKEDPDB threshold'
# saves AI_per_residue DO_AI_per_threshold and print summary statistics

# calculate density occupancy and atomic inclusion
# with heavy atoms and vdw radius
# for a range of threshold including recommended threshold
# will also calculate per residue AI for thresholds inputted
...
thresholds = [x/1000 for x in range(round(min_value*1000),
                                    round(max_value*1000),
                                    round(std_value*1000/10))]
...
BACKBONE_ATOMS = ['P', 'OP1', 'OP2', "O5'", "C5'",
...
# remove hydrogen and get backbone index
...
for res in residues:
    ...
    num_res_backbone_threshold = np.count_nonzero(
        res_values_backbone > specific_threshold)
    num_res_base_threshold = np.count_nonzero(
        res_values_base > specific_threshold)
...
np.savetxt(f'{sys.argv[4]}_threshold{specific_threshold}_AI_per_residue.csv',
           np.array([residues, AI_res, AI_res_backbone, AI_res_base]).T,
           delimiter=' ',
           header='residue atom_inclusion atom_inclusion_backbone atom_inclusion_base',
           comments='', fmt='%15.15f')
...
for i, a_coord in enumerate(heavy_atom_coords):
    ...
    min_search_ijk = mrc[0].data.xyz_to_ijk(
        min_search_coord).round().astype(int)
    max_search_ijk = mrc[0].data.xyz_to_ijk(
        max_search_coord).round().astype(int)
    search_ijk = np.array(list(product(range(min_search_ijk[0], max_search_ijk[0]+1),
                                       range(min_search_ijk[1], max_search_ijk[1]+1),
                                       range(min_search_ijk[2], max_search_ijk[2]+1))))
    ...
    for index in dist_within_atom:
        ...
        if tuple(search_ijk[index]) not in index_already_added:
            maps_value = mrc[0].data.matrix()[max(min(mrc_data_shape[0]-1, search_ijk[index][0]), 0),
                                              max(min(mrc_data_shape[1]-1, search_ijk[index][1]), 0),
                                              max(0, min(mrc_data_shape[2]-1, search_ijk[index][2]))]
...
for threshold in thresholds:
    num_total_vertex_threshold = np.count_nonzero(
        all_vertex_values > threshold)
    num_near_atom_vertex_threshold = np.count_nonzero(
        np.array(near_atom_vertex_values) > threshold)
    num_near_atom_vertex_threshold_backbone = np.count_nonzero(
        np.array(near_atom_vertex_values_backbone) > threshold)
    num_near_atom_vertex_threshold_base = np.count_nonzero(
        np.array(near_atom_vertex_values_base) > threshold)
    num_atom_threshold = np.count_nonzero(heavy_atom_values > threshold)
    num_backbone_threshold = np.count_nonzero(
        heavy_backbone_values > threshold)
    ...
    DOs_backbone.append(
        num_near_atom_vertex_threshold_backbone/num_total_vertex_threshold)
    DOs_base.append(num_near_atom_vertex_threshold_base /
                    num_total_vertex_threshold)
...
# save and exit
np.savetxt(f'{sys.argv[4]}_DO_AI_per_threshold.csv',
           np.array([thresholds, DOs, DOs_backbone, DOs_base,
                     AIs, AIs_backbone, AIs_base]).T,
           delimiter=' ',
           header='threshold density_occupancy density_occupancy_backbone density_occupancy_base atom_inclusion atom_inclusion_backbone atom_inclusion_base',
           comments='', fmt='%15.15f')
run(session, 'exit')
